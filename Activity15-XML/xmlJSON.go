...
func init() {
	settingData, _ := ioutil.ReadFile("settings.xml")
...
func main() {
	...
	log.Fatal(http.ListenAndServe(settings.NIP+":"+settings.NPort, nil))
}

func weatherpage(ress http.ResponseWriter, req *http.Request) {
	result, err := http.Get(settings.URLJSON)
